...
unsigned char QMI8658_write_regs(unsigned char reg, unsigned char *value,
                                 unsigned char len) {
    int i, ret;
...
unsigned char QMI8658_read_reg(unsigned char reg, unsigned char *buf,
                               unsigned short len) {
    unsigned char ret = 0;
    unsigned int retry = 0;
...
void QMI8658_config_ae(enum QMI8658_AeOdr odr) {
    // QMI8658_config_acc(QMI8658AccRange_8g, AccOdr_1000Hz, Lpf_Enable,
    // St_Enable); QMI8658_config_gyro(QMI8658GyrRange_2048dps, GyrOdr_1000Hz,
    // Lpf_Enable, St_Enable);
    QMI8658_config_acc(QMI8658_config.accRange, QMI8658_config.accOdr,
                       QMI8658Lpf_Enable, QMI8658St_Disable);
    QMI8658_config_gyro(QMI8658_config.gyrRange, QMI8658_config.gyrOdr,
                        QMI8658Lpf_Enable, QMI8658St_Disable);
...
void QMI8658_read_xyz(float acc[3], float gyro[3], unsigned int *tim_count) {
    ...
    // rad/s
    gyro[AXIS_X] =
        (float)(raw_gyro_xyz[AXIS_X] * 0.01745f) / gyro_lsb_div; // *pi/180
...
void QMI8658_enableWakeOnMotion(void) {
    ...
    womCmd[0] = QMI8658Register_Cal1_L; // WoM Threshold: absolute value in mg
                                        // (with 1mg/LSB resolution)
    womCmd[1] = threshold;
    womCmd[2] = (unsigned char)interrupt | (unsigned char)initialState |
                (blankingTime & blankingTimeMask);
...
void QMI8658_enableSensors(unsigned char enableFlags) {
    ...
    QMI8658_write_reg(QMI8658Register_Ctrl7,
                      enableFlags & QMI8658_CTRL7_ENABLE_MASK);
}
...
void QMI8658_Config_apply(struct QMI8658Config const *config) {
    ...
    if (fisSensors & QMI8658_CONFIG_AE_ENABLE) {
        ...
        if (config->inputSelection & QMI8658_CONFIG_ACC_ENABLE) {
            QMI8658_config_acc(config->accRange, config->accOdr,
                               QMI8658Lpf_Enable, QMI8658St_Disable);
        }
        if (config->inputSelection & QMI8658_CONFIG_GYR_ENABLE) {
            QMI8658_config_gyro(config->gyrRange, config->gyrOdr,
                                QMI8658Lpf_Enable, QMI8658St_Disable);
        }
    }
...
unsigned char QMI8658_init(void) {
    ...
    unsigned char QMI8658_slave[2] = {QMI8658_SLAVE_ADDR_L,
