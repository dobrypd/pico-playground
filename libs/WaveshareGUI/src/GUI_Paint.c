...
void Paint_NewImage(UBYTE *image, UWORD Width, UWORD Height, UWORD Rotate,
    ...
    //    Paint.HeightByte); printf(" LCD_WIDTH / 8 = %d\r\n",  122 / 8);
...
void Paint_DrawNum(UWORD Xpoint, UWORD Ypoint, double Nummber, sFONT *Font,
                   UWORD Digit, UWORD Color_Foreground,
                   UWORD Color_Background) {
    // Note: there was a bug in original code. Replaced with:
    char outStr[ARRAY_LEN];
    snprintf(outStr, ARRAY_LEN, "%.*f", Digit, Nummber);
    Paint_DrawString_EN(Xpoint, Ypoint, (const char *)outStr, Font,
    ...
    for (j = 0; j < H_Image; j++) {
        for (i = 0; i < W_Image; i++) {
            if (xStart + i < Paint.WidthMemory &&
                yStart + j <
                    Paint.HeightMemory) // Exceeded part does not display
    ...
    for (j = 0; j < H_Image; j++) {
        for (i = 0; i < W_Image; i++) {
            if (xStart + i < Paint.HeightMemory &&
                yStart + j <
                    Paint.WidthMemory) // Exceeded part does not display
                Paint_SetPixel(xStart + i, yStart + j,
                               (*(image + j * W_Image * 2 + i * 2 + 1)) << 8 |
                                   (*(image + j * W_Image * 2 + i * 2)));
            // Using arrays is a property of sequential storage, accessing the
    ...
    for (y = 0; y < Paint.HeightByte; y++) {
        for (x = 0; x < Paint.WidthByte; x++) { // 8 pixel =  1 byte
            ...
            Paint.Image[Addr] = (unsigned char)
                image_buffer[Addr + (Paint.HeightByte) * Paint.WidthByte *
                                        (Region - 1)];
        }
    }
}
...
void Paint_BmpWindows(unsigned char x, unsigned char y,
                      const unsigned char *pBmp, unsigned char chWidth,
